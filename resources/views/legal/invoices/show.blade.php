...
<body class="bg-gradient-to-tr from-indigo-50 to-blue-100 min-h-screen flex">
    ...
    <aside class="w-64 bg-indigo-600 text-white flex flex-col shadow-lg">
        ...
        <nav class="flex-1 p-4 space-y-3">
            <a href="{{ route('legal.dashboard') }}"
                class="block py-2 px-4 rounded hover:bg-indigo-500 transition">داشبورد</a>
            <a href="#" class="block py-2 px-4 rounded hover:bg-indigo-500 transition">پروفایل</a>
            <a href="{{ route('legal.invoices.index') }}"
                class="block py-2 px-4 rounded bg-indigo-500 hover:bg-indigo-600 transition">فاکتورها</a>
    ...
    <main class="flex-1 p-6">
        ...
        <section class="bg-white shadow rounded-lg p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <td class="px-4 py-2 border-b">{{ \Carbon\Carbon::parse($invoice->invoice_date)->format('Y-m-d') }}
                    </td>
                    <p class="text-gray-600"><strong>نوع فاکتور:</strong> {{ $invoice->invoice_type }}</p>
                </div>
                <div>
                    <p class="text-gray-600"><strong>مبلغ کل:</strong> {{ number_format($invoice->total_amount) }} تومان
                    </p>
        ...
        <div class="mt-6">
            <a href="{{ route('legal.invoices.index') }}"
                class="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700">بازگشت به لیست فاکتورها</a>
        </div>
    </main>
</body>
